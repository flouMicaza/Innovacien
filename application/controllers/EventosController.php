...
class EventosController extends CI_Controller {
	...
	public function irEventos(){
		...
		$ans=$this->seguridadModel->personaEnProyecto($data['mail'],$nombrecito);
	...
	public function actualizarAsistencia(){
		...
		if($ans!=null || $data['tipo']=='administrador'){
			...
			if($asistencia=='si'){
				$this->load->model('eventosModel');
				$this->eventosModel->actualizarAsistencia($data['mail'],$data['idevento'],1);
				$data['respondido']='si';
				...
				$this->load->model('eventosModel');
				$this->eventosModel->actualizarAsistencia($data['mail'],$data['idevento'],0);
				$data['respondido']='si';
	...
	public function actualizarFuturaAsistencia(){
		...
		if($ans==null){
			...
			if($asistencia == 'si'){
				...
				$this -> eventosModel -> ingresarPagoEvento($data['mail'],$idEvento,$comentario,$datos_evento['pago']);
				$datos_evento = $this->eventosModel->infoEventoParticipante($idEvento,$data['mail']);
				}
			...
			if ($asistencia == 'no'){
				if($datos_evento['asistencia'] != 0){
					$this -> eventosModel -> quitarPagoEvento($data['mail'],$idEvento);
				}
